...
\begin{enumerate}
    ...
    {\color{blue}
        ...
        After acquiring $\vec{y}$, we can acquire $\vec{z}$ with the third, the fourth, and the fifth equalities. Thus,
        \begin{align}
        ...
        Solving the equalities above, we have $\vec{z}_1 + 2\vec{z}_2 = 1$. 
        Let $\vec{z}_1 = 1 - 2t$ and $\vec{z}_2 = t$. We can express them as,
        \begin{align}
            \vec{z} = \begin{bmatrix} 1 - 2t \\ t \end{bmatrix}
        \end{align}

        Since the $\vec{z}$ that we acquire here is the same with the $\vec{z}$ we got from its normal equation, 
        thus the premise, $\vec{z} \in \mathbb{R}^2$ is a least square solution for $A\vec{x} = \vec{b}$ if and only if $\vec{z}$ is a part of the solution to the larger linear system, holds.
    }
